...
export default function BorrowsScreen({ navigation }) {
  ...
  const loadBorrows = async () => {
    try {
      const response = await apiService.admin.getBorrows();
      // Handle both direct array and response object
      const data = Array.isArray(response) ? response : (response?.borrows || response?.data || []);
      setBorrows(data);
    } catch (error) {
      console.error('Error loading borrows:', error);
      Alert.alert('Error', error.response?.data?.message || error.message || 'Failed to load borrows');
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const handleIssue = async () => {
    ...
    try {
      await apiService.admin.issueBook({
    ...
    Alert.alert(
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              await apiService.admin.returnBook(borrow.id);
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async (days) => {
            ...
            try {
              await apiService.admin.extendBorrow(borrow.id, additionalDays);
  ...
  const openIssueModal = async () => {
    try {
      const [usersData, booksResponse] = await Promise.all([
        ...
        apiService.admin.getBooks(1, 1000, ''), // Get all books (1000 per page)
      ]);
      
      // Extract books from response (it might be paginated)
    ...
    <View style={styles.card}>
      <View style={styles.cardContent}>
        ...
        <View style={styles.metaRow}>
          ...
          <Text style={[styles.date, item.status === 'borrowed' && new Date(item.due_date) < new Date() ? styles.overdue : null]}>
          ...
          <View style={styles.statusRow}>
            {(() => {
              ...
              if (daysDiff < 0) {
                ...
                const finePerDay = item.fine_per_day || 10;
                ...
                return (
                  <>
                    ...
                    <Text style={styles.fineText}>Fine: ₹{estimatedFine.toFixed(2)}</Text>
                  </>
                );
              } else if (daysDiff === 0) {
                return <Text style={[styles.status, styles.dueToday]}>Due Today</Text>;
              } else {
                return <Text style={[styles.status, styles.active]}>{daysDiff} day(s) left</Text>;
              }
            })()}
          </View>
        )}
