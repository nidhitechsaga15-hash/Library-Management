...
class BorrowController extends Controller
{
    ...
    public function create()
    {
        ...
        return view('staff.borrows.create', compact('books'));
    }
    ...
    public function issue(Request $request)
    {
        ...
        if (!$user) {
            return redirect()->back()
                ->withInput()
                ->with('error', 'Student not found with the provided ID/Email!');
        }

        if ($user->role !== 'student') {
            return redirect()->back()
                ->withInput()
                ->with('error', 'Only students can borrow books!');
        }

        if (!$user->is_active) {
            return redirect()->back()
                ->withInput()
                ->with('error', 'Student account is inactive!');
        }
        ...
        if ($existingBorrow) {
            return redirect()->back()
                ->withInput()
                ->with('error', 'Student already has this book borrowed!');
        }
        ...
        $finePerDay = \App\Helpers\FineHelper::getFinePerDayByDuration($validated['issue_duration_days']);
    ...
    public function showReturn(Borrow $borrow)
    {
        ...
        return view('staff.borrows.return', compact('borrow', 'fineAmount', 'daysOverdue', 'returnDate'));
    }
    ...
    public function overdue()
    {
        ...
        return view('staff.borrows.overdue', compact('overdueBorrows', 'totalOverdue', 'finePerDay'));
    }
